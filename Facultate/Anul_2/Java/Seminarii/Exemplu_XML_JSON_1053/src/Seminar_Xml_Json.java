...
//                System.out.println(nodTranzactie.getTextContent());
    ...
    public static void salvareXml(String caleFisier, List<Produs> produse) throws Exception {
        ...
        var nodRadacina=document.createElement("produse");
        document.appendChild(nodRadacina);

        for(Produs produs : produse)
        {
            var nodProdus=document.createElement("produs");
            var nodCod=document.createElement("cod");
            nodCod.setTextContent(String.valueOf(produs.getCod()));
            ...
            for(Tranzactie tranzactie : produs.getTranzactii())
            {
                ...
                nodTranzactie.setAttribute("tip",String.valueOf(tranzactie.getTip()).toLowerCase());
                nodTranzactie.setAttribute("cantitate",String.valueOf(tranzactie.getCantitate()));

                nodTranzactii.appendChild(nodTranzactie);
            }
            nodProdus.appendChild(nodTranzactii);
            nodRadacina.appendChild(nodProdus);
        }
    ...
    public static void salvareJson(String caleFisier, List<Produs> produse)
        ...
        JSONArray jsonProduse = new JSONArray();
        for (Produs produs : produse) {
            var jsonProdus = new JSONObject();
            jsonProdus.put("cod", produs.getCod());
            jsonProdus.put("denumire", produs.getDenumire());

            JSONArray jsonTranzactii = new JSONArray();
            for (Tranzactie tranzactie : produs.getTranzactii()) {
                var jsonTranzactie = new JSONObject();
                jsonTranzactie.put("tip", String.valueOf(tranzactie.getTip()).toLowerCase());
    ...
    public static List<Produs> citireJson(String caleFisier) throws Exception {
        ...
        try(FileInputStream fisier=new FileInputStream(caleFisier))
        {
            var tokener=new JSONTokener(fisier);
            JSONArray jsonProduse = new JSONArray(tokener);
            for(int i=0;i<jsonProduse.length();i++)
            {
                var jsonProdus= jsonProduse.getJSONObject(i);
                List<Tranzactie> tranzactii= StreamSupport.stream(jsonProdus.getJSONArray("tranzactii")
                                .spliterator(),false)
                        .map(item->(JSONObject)item)
                        .map(item->new Tranzactie(TipTranzactie.valueOf(item.getString("tip").toUpperCase()),
                                item.getInt("cantitate")))
                        .collect(Collectors.toList());

                Produs produs=new Produs(jsonProdus.getInt("cod"),
                        jsonProdus.getString("denumire"),tranzactii);
