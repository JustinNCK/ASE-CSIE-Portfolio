...
from Selectii import s_turneu2, s_superelitista
from Recombinari import r_a_simpla, r_a_sing, r_a_total
from Mutatii import m_reale_neunif


def GA_RucsacCf(numef,cmax,dim,pr,a,pm,T,nmax):
	...
	#    a - ponderea pentru recombinarea aritmetica
    ...
    for i in range(nmax):
        # selectie parinti
        parinti = s_turneu2(pop)
        # recombinare
        desc=recombinare(parinti,pr,a,profit,cost,cmax)
        # mutatie
        descm=mutatie(desc,pm,T,profit,cost,cmax)
        # selectie generatie urmatoare
        pop = s_superelitista(pop, descm)
        # alte operatii
...
def recombinare(parinti,pr,a,profit,cost,cmax):
    ...
    for i in range(0,dim,2):
        ...
        #c1,c2,da=r_a_simpla(x,y,pr,a)
        c1,c2,da=r_a_sing(x,y,pr,a)
        #c1,c2,da=r_a_total(x,y,pr,a)
        if numpy.dot(c1,cost)<=cmax and da:
            desc[i,:n-1]=c1
            desc[i,n-1]=f_obiectiv(c1,profit)
        if numpy.dot(c2,cost)<=cmax and da:
            desc[i+1,:n-1]=c2
            desc[i+1,n-1]=f_obiectiv(c2,profit)
    return desc

def arata(sol,v):
    # vizualizare rezultate RucsacCf
...
if __name__=="__main__":
    s, v = GA_RucsacCf("date_RC1.txt",30,100,0.7,0.1,0.3,0.7,300)
    grafic.pause(20)
    #print("Solutia:",s)
    #print("Profit: ",v[-1])
