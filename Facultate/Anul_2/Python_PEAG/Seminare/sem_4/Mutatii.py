...
def m_int_fluaj(x,pm,a,b,max):
    ...
    for i in range(m):
        ...
        if r<pm:
            ...
            #f=numpy.random.choics([-1,1]) #pentru fluaj cu magnitudinea 1
            y[i]=y[i]+f
            if y[i]<a:
                y[i]=a
            if y[i]>b:
...
def m_perm_schimb(x,pm):
    ...
    if r<pm:
        da=1
        m = len(x)
        #selectare pozitii distincte
        p = numpy.random.randint(0, m, 2)
        while p[0] == p[1]:
            p[1] = numpy.random.randint(0,m)
        #p=numpy.random.choice(m,size=2,replace=False)  #varianta selectare pozitii distincte
        #p=numpy.random.permutation(m)[:2]              #varianta selectare pozitii distincte
        y[p[1]]=x[p[0]]
        y[p[0]]=x[p[1]]
    return y, da
...
def m_perm_inversiune(x,pm):
    ...
    if r<pm:
        ...
        #y[p[0]:p[1]+1]=x[p[1]:p[0]-1:-1]   #varianta pentru  inversare secventa
    return y, da
