...
void dezalocareTabelaHash(Nod** HT);
...
int main()
{
	...
	unsigned int idDeSters = 1097;
	printf("\nStergere client cu ID %u...", idDeSters);
	stergereClientTabela(HashTable, idDeSters);


	pClient = cautareClientCheie(HashTable, idDeSters);
	if (pClient == NULL) {
		printf("\nClientul a fost sters cu succes.");
	}
	else {
		printf("\nClientul inca exista.");
	}

	dezalocareTabelaHash(HashTable);
	HashTable = NULL;

	printf("\n\nTabela a fost stearsa");
